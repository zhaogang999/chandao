...
<div id='unlinkBugList'>
  <form method='post' id='unlinkedBugsForm' target='hiddenwin' action='<?php echo $this->createLink('patchbuild', 'linkBug', "buildID={$build->id}&from=$from&browseType=$browseType&param=$param");?>'>
    <table class='table table-condensed table-hover table-striped tablesorter table-fixed table-selectable'> 
      ...
      <?php if(strpos(",{$build->linkBugs},", ",$bug->id,") !== false) continue;?>
      <tr>
        <td class='cell-id'>
          <input type='checkbox' name='bugs[<?php echo $bug->id?>]'  value='<?php echo $bug->id;?>' />
      ...
      <tr>
        <td colspan='6' class='text-left'>
          <?php if(count($allBugs))
          {
              ...
              echo html::selectButton() . html::submitButton($lang->patchbuild->linkBug);
