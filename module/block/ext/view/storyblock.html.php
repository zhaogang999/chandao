...
<table class='table tablesorter table-data table-hover block-story table-fixed'>
  ...
  if ($story->projectID)
  {
    $viewLink = $this->createLink('story', 'view', "id={$story->id}&version={$story->version}&from=project&param={$story->projectID}");
  }
  else
  {
    $viewLink = $this->createLink('story', 'view', "id={$story->id}");
  }
