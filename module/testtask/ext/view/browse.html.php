...
<table class='table tablesorter table-fixed' id='taskList'>
  ...
  <?php $vars = "productID=$productID&branch=$branch&type=$scope,$status&orderBy=%s&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}"; ?>
    <tr>
      <th class='w-40px text-left'>   <?php common::printOrderLink('id',      $orderBy, $vars, $lang->idAB);?></th>
      <th class='text-left'><?php common::printOrderLink('name',    $orderBy, $vars, $lang->testtask->name);?></th>
      <th class='w-100px'>        <?php common::printOrderLink('product', $orderBy, $vars, $lang->testtask->product);?></th>
      <?php if($this->config->global->flow != 'onlyTest'):?>
      <th class='w-180px'>        <?php common::printOrderLink('project', $orderBy, $vars, $lang->testtask->project);?></th>
      <?php endif;?>
      <th width='275' class='text-left'>        <?php common::printOrderLink('build',   $orderBy, $vars, $lang->testtask->build);?></th>
      <th class='w-60px'> <?php common::printOrderLink('owner',   $orderBy, $vars, $lang->testtask->owner);?></th>
      <th class='w-80px text-left'><?php common::printOrderLink('begin',   $orderBy, $vars, $lang->testtask->begin);?></th>
      <th class='w-80px text-left'><?php common::printOrderLink('end',     $orderBy, $vars, $lang->testtask->end);?></th>
      <th class='w-50px text-left'> <?php common::printOrderLink('status',  $orderBy, $vars, $lang->statusAB);?></th>
