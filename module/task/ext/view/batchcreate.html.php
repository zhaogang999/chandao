  ...
  <table class='table table-form table-fixed with-border' id="tableBody">
    <thead>
      <tr class='text-center'>
        ...
        <!--禅道任务增加关键字字段；需求：858 批量添加任务，批量编辑任务增加关键字字段；行：51-52-->
        <th class='<?php echo zget($visibleFields, 'keywords', ' hidden')?>'><?php echo $lang->task->keywords?></th>
      </tr>
    </thead>
    ...
    <?php for($i = 0; $i < $config->task->batchCreate; $i++):?>
      ...
      <tr>
        ...
        <td <?php echo zget($visibleFields, 'pri', "class='hidden'")?>><?php echo html::select("pri[$i]", (array)$lang->task->priList, $parent != 0 ? $parentTask->pri : $pri, 'class=form-control');?></td>
        <!--禅道任务增加关键字字段；需求：858 批量添加任务，批量编辑任务增加关键字字段；行：101-102-->
...
<table class='hide' id='trTemp'>
  ...
  <tr>
    ...
    <!--禅道任务增加关键字字段；需求：858 批量添加任务，批量编辑任务增加关键字字段；行：134-135-->
