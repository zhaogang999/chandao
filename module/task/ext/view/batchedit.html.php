  ...
  <form class='form-condensed' method='post' target='hiddenwin' action="<?php echo inLink('batchEdit', "projectID={$projectID}")?>">
    <table class='table table-form table-fixed with-border'>
      ...
      <tr>
        ...
        <!--增加实际开始-->
        <th class='w-100px<?php echo zget($visibleFields, 'realStarted', ' hidden')?>'><?php echo $lang->task->realStarted?></th>
        ...
        <!--禅道任务增加关键字字段；需求：858 批量添加任务，批量编辑任务增加关键字字段；行：55-56-->
        <th class='w-100px<?php echo zget($visibleFields, 'keywords', ' hidden')?>'><?php echo $lang->task->keywords?></th>
      </tr>
      </thead>
      <tbody>
      <?php foreach($taskIDList as $taskID):?>
        ...
        <tr class='text-center'>
          ...
          <!--//禅道任务增加关键字字段；需求：858 批量添加任务，批量编辑任务增加关键字字段；行：95-96-->
          <td <?php echo zget($visibleFields, 'keywords', "class='hidden'")?>><?php echo html::input("keywords[$taskID]", $tasks[$taskID]->keywords, 'class=form-control');?></td>
        </tr>
      <?php endforeach;?>
      </tbody>
      <tfoot>
